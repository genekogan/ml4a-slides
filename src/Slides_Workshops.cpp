...
//    s->addText(h2, ofColor(0), "It is the same in all wars; the soldiers do the fighting, the journalists do the shouting, and no true patriot ever gets near a front-line trench, except on the briefest of propaganda-tours.\n\nGeorge Orwell\nHomage to Catalonia, 1938", 0.05, 0.15, 0.75);
    s->addText(h3, ofColor(0), "It was the first time I had ever been in a town where the working class was in the saddle. Practically every building of any size had been seized by the workers and was draped with red flags or with the red and black flag of the Anarchists; every wall was scrawled with the hammer and sickle and with the initials of the revolutionary parties; almost every church had been gutted and its images burnt. Churches here and there were being systematically demolished by gangs of workmen. Every shop and caf̩ had an inscription saying that it had been collectivised; even the bootblacks had been collectivised and their boxes painted red and black. Waiters and shop-walkers looked you in the face and treated you as an equal.\n\nGeorge Orwell\nHomage to Catalonia, 1938", 0.05, 0.15, 0.75);
...
void ofApp::WorkshopParsons() {
    ...
    s = slideshow.addSlide("ml4a @ Parsons, Paris, 28 Feb 2017");
    s->addText(h2, ofColor(0), "@genekogan", 0.1, 0.2, 0.5);
    s->addText(h2, ofColor(0), "genekogan.com", 0.1, 0.3, 0.5);
    s->addText(h2, ofColor(0), "ml4a.github.io", 0.6, 0.2, 0.5);
    s->addText(h2, ofColor(0), "@ml4a_", 0.6, 0.3, 0.5);
    ...
    s = slideshow.addSlide("what we will cover");
    s->addText(h3, ofColor(0), " - Tiny history, relevance", 0.1, 0.1, 0.8);
    s->addText(h3, ofColor(0), " - How neural nets work", 0.1, 0.2, 0.8);
    s->addText(h3, ofColor(0), " - Playing with neural nets and wekinator", 0.1, 0.3, 0.8);
    s->addText(h3, ofColor(0), " - How convnets work", 0.1, 0.4, 0.8);
    s->addText(h3, ofColor(0), " - Overview of ml4a-ofx and ml4a-guides", 0.1, 0.5, 0.8);
    
    
    s = slideshow.addSlide("about the workshop");
    s->addText(h3, ofColor(0), " + Breadth vs. depth", 0.1, 0.1, 0.8);
    s->addText(h3, ofColor(0), " - This workshop will create more questions than answers", 0.1, 0.2, 0.8);
    ...
    s = slideshow.addSlide("Tiny history to deep learning");
    s->addText(h2, ofColor(0), "70s-90s", 0.1, 0.0, 0.7);
    s->addText(h2, ofColor(0), " - AI Winter", 0.1, 0.1, 0.7);
    s->addText(h2, ofColor(0), " - \"Machine learning\" grows out of computational stats applied to AI", 0.1, 0.2, 0.8);
    s->addText(h2, ofColor(0), " - Other methods prevail: SVMs, decision forests, Boosting, etc", 0.1, 0.35, 0.8);
    s->addText(h2, ofColor(0), " - Applied machine learning characterized by \"expert\" handcrafted feature-engineering", 0.1, 0.5, 0.8);
    s->addText(h2, ofColor(0), "2010s", 0.1, 0.72, 0.8);
    s->addText(h2, ofColor(0), " - Emergence of deep learning", 0.1, 0.82, 0.8);
    //
    //    s = slideshow.addSlide("Emergence of deep learning");
    //    s->addText(h2, ofColor(0), " - 1980s: Geoff Hinton applies backpropagation to train multiple layer neural nets", 0.1, 0.0, 0.8);
    //    s->addText(h2, ofColor(0), " - 1998: LeNet + handwritten check detection", 0.1, 0.1, 0.8);
    ...
    s->addImage("rosf", "/Users/gene/bin/misc/ROSF/Screen Shot 2017-03-07 at 5.04.47 PM.png", 0, 0, 1, 1);
...
void ofApp::WorkshopSpektrum2() {
    
    
    /////////////////////////////////////////////////
    // INTRO
    
    s = slideshow.addSlide("ml4a @ Spektrum, Berlin, 7 Mar 2017");
    s->addText(h2, ofColor(0), "@genekogan", 0.1, 0.2, 0.5);
    s->addText(h2, ofColor(0), "genekogan.com", 0.1, 0.3, 0.5);
    s->addText(h2, ofColor(0), "ml4a.github.io", 0.6, 0.2, 0.5);
    s->addText(h2, ofColor(0), "@ml4a_", 0.6, 0.3, 0.5);
    ...
    // RISF + RISL
    // ROSF + RISDL
    // ConvnetClassifier + ConvnetOSC
    // DoodleClassifier
    // FaceClassifier / FaceRegression
    //  - clm tracker + p5js
    // AudioTsne
    
    s = slideshow.addSlide("what we will cover");
    s->addText(h3, ofColor(0), " - ConvnetClassifier + ConvnetOSC", 0.1, 0.0, 0.8);
    s->addText(h3, ofColor(0), " - DoodleClassifier", 0.1, 0.08, 0.8);
    s->addText(h3, ofColor(0), " - FaceClassifier + FaceRegression", 0.1, 0.16, 0.8);
    s->addText(h3, ofColor(0), " - Image classification (darknet)", 0.1, 0.24, 0.8);
    s->addText(h3, ofColor(0), " - YOLO object detection (darknet)", 0.1, 0.32, 0.8);
    s->addText(h3, ofColor(0), " - Reverse Image Search", 0.1, 0.4, 0.8);
    s->addText(h3, ofColor(0), " - Reverse Object Search", 0.1, 0.48, 0.8);
    s->addText(h3, ofColor(0), " ? Audio t-SNE", 0.1, 0.56, 0.8);
    s->addText(h3, ofColor(0), " ? Image t-SNE", 0.1, 0.64, 0.8);
    ...
    s = slideshow.addSlide("ML4A-ofx");
    s->addText(h3, ofColor(0), " - ConvnetClassifier + ConvnetOSC", 0.1, 0.02, 0.8);
    s->addText(h3, ofColor(0), " - DoodleClassifier", 0.1, 0.1, 0.8);
    s->addText(h3, ofColor(0), " - FaceClassifier / FaceRegression", 0.1, 0.18, 0.8);
    s->addMovie("convnetosc", "/Users/gene/bin/misc/workshops/faceclassification-p5js.mp4", true, 0, 0.24, 1, 0.76);
    ...
    //    s = slideshow.addSlide("Reverse Image Search Fast/Live");
    //    ReverseImageSearchFastDemo *risfd = new ReverseImageSearchFastDemo(s, "Reverse Image Search Fast", 0, 0, 1, 1);
    //    risfd->setCcvReference(&ccv);
    ////    risfd->load("/Users/gene/bin/ml4a/ReverseImageSearch/data_small.dat");
    //    risfd->load("/Users/gene/bin/ml4a/ReverseImageSearch/data_vecs50k.dat");
    ...
    //    rosd->load("/Users/gene/Teaching/ML4A/ml4a-ofx/models/mscoco_145k_rp64.dat");
    //    rosd->runKDTree();
    //    s->setBackgroundFunction(this, &ofApp::drawBgBlack);
    //    s->addAction(rosd);
...
void ofApp::WorkshopStrelkaNeuralNets(){
    ...
    s = slideshow.addSlide("Part 2");
    s->addText(h2, ofColor(255), "This lecture will be unsatisfying", 0.1, 0.1, 0.7);
    ...
    ReverseObjectSearchFastDemo *rosd = new ReverseObjectSearchFastDemo(s, "Reverse Object Search Fast", 0, 0, 1, 1);
    rosd->setDarknetReference(&darknet);
    rosd->load("/Users/gene/Teaching/ML4A/ml4a-ofx/models/mscoco_145k_rp64.dat", "/Users/gene/Teaching/ML4A/ml4a-ofx/apps/ReverseImageSearchFast/bin/data/mscoco");
    rosd->runKDTree();
    s->setBackgroundFunction(this, &ofApp::drawBgBlack);
    s->addAction(rosd);
